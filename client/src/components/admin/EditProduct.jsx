...
export default function EditProduct({ close, pdata, fetchProducts }) {
  ...
  const handleProductSubmit = async (e) => {
    try {
      ...
      const { data: res } = await Axios({
        ...apiSummary.updateProduct,
        url: apiSummary.updateProduct.url.replace(":id", pdata._id),
        data: data,
      });
      if (res.success) {
  ...
  return (
    <section className="fixed top-0 bottom-0 left-0 right-0 z-50 bg-black bg-opacity-70 p-4">
      <div className="bg-white p-4 max-w-2xl w-full rounded mx-auto overflow-y-auto h-full min-h-[95vh]">
        <>
          <div className="p-2 bg-white shadow-md flex items-center justify-between">
            <h2 className="font-semibold">Edit Products</h2>
            <button
              onClick={close}
              className="w-fit block ml-auto animate-pulse"
            >
              <IoClose size={24} />
            </button>
          </div>
                  ...
                  <div>
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        const exist = data.brand_id.find(
                          (b) => b._id === value
                        );
                        if (exist) return;
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-1">
                ...
                <div className="grid gap-1">
                  <label htmlFor="discount">
                    Product Discount{" "}
                    <span className="text-red-600">(if have)</span>
                  </label>
                  <input
                    type="number"
                    id="discount"
