...
export default function EditBand({ close, data: editData, fetchBrands }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data: res } = await Axios({
        ...apiSummary.updateBrand,
        url: apiSummary.updateBrand.url.replace(":id", editData._id),
        data: data,
      });
      if (res.success) {
    ...
    <div className="fixed top-0 bottom-0 left-0 right-0 bg-neutral-800 bg-opacity-70 p-4 flex items-center justify-center z-50">
      <div className="bg-white w-full max-w-2xl p-4 rounded">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold">Edit Product Brand</h2>
        ...
        <form onSubmit={handleSubmit} className="py-5">
          ...
          <div className="pt-4">
            ...
            >
              {submitting ? "Submitting..." : "Update Brand"}
            </button>
          </div>
        </form>
      </div>
